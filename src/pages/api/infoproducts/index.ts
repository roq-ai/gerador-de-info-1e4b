...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getInfoproducts() {
    const data = await prisma.infoproduct
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'infoproduct'));
    return res.status(200).json(data);
  }

  async function createInfoproduct() {
    await infoproductValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.infoproduct.create({
      data: body,
    });
    return res.status(200).json(data);
  }
